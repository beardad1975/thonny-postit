...
from .common import symbol_nt_dict

class SymbolPostit(Postit):
    def __init__(self, master, symbol_code):
        super().__init__(master)

        self.symbol = symbol_nt_dict[symbol_code]
        self.default_symbol = self.symbol

        self.update()

        self.popup_menu = tk.Menu(self, tearoff=0)
        self.popup_menu.add_command(label="貼上",command=self.post)
        self.popup_menu.add_command(label="貼上(前後加上空白)",
                                    command=lambda :self.post(space_surround_inline=True))
    ...
    def on_edit(self):
        self.edit_popup = SymbolEditWindow(self)

    def update(self):
        self.set_code_display(self.symbol.code)
        self.set_code(self.symbol.code)
        self.set_note(self.symbol.note)




class SymbolEditWindow(tk.Toplevel):
